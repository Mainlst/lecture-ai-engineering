...
def raw_data():
    ...
    if not os.path.exists(DATA_PATH):
        ...
        if missing_cols:
            raise ValueError(
                f"必要なカラム {missing_cols} がDataFrameに存在しません。利用可能なカラム: {df.columns.tolist()}"
            )
    ...
    df_loaded.columns = [
        col.lower() for col in df_loaded.columns
    ]  # 読み込んだCSVもカラム名を小文字に
    return df_loaded
...
def preprocessor(raw_data):  # raw_dataフィクスチャに依存
    ...
    for col in numeric_features + categorical_features:
        if col not in raw_data.columns:
            raise ValueError(
                f"前処理に必要なカラム '{col}' がデータに存在しません。利用可能なカラム: {raw_data.columns.tolist()}"
            )
...
def trained_model_and_test_data(raw_data, preprocessor):  # raw_dataとpreprocessorに依存
    ...
    if "survived" not in raw_data.columns:
        raise ValueError(
            f"目的変数 'survived' がデータに存在しません。利用可能なカラム: {raw_data.columns.tolist()}"
        )
...
def test_model_accuracy(trained_model_and_test_data):
    ...
    assert (
        accuracy >= 0.70
    ), f"モデルの精度 ({accuracy:.4f}) が期待値 (0.70) より低いです"  # 閾値を少し現実的に
...
def test_model_inference_time(trained_model_and_test_data):
    ...
    inference_time_avg_ms = (
        (inference_time_total / len(X_test)) * 1000 if len(X_test) > 0 else 0
    )
    ...
    assert (
        inference_time_total < 5.0
    ), f"総推論時間 ({inference_time_total:.4f}秒) が長すぎます"


def test_model_reproducibility(
    raw_data, preprocessor
):  # trained_model_and_test_dataは使わない
    ...
    numeric_features = ["age", "pclass", "sibsp", "parch", "fare"]
    categorical_features = ["sex", "embarked"]
    numeric_transformer_new = Pipeline(
        steps=[
            ("imputer", SimpleImputer(strategy="median")),
            ("scaler", StandardScaler()),
        ]
    )
    categorical_transformer_new = Pipeline(
        steps=[
            ("imputer", SimpleImputer(strategy="most_frequent")),
            ("onehot", OneHotEncoder(handle_unknown="ignore")),
        ]
    ...
    preprocessor_new = ColumnTransformer(
        transformers=[
            ("num", numeric_transformer_new, numeric_features),
...
def test_performance_regression():
    ...
    if not os.path.exists(CURRENT_METRICS_PATH):
        pytest.skip(
            f"現在のメトリクスファイル ({CURRENT_METRICS_PATH}) が見つかりません。"
        )
    ...
    if not os.path.exists(PREVIOUS_METRICS_PATH):
        print(
            f"過去のメトリクスファイル ({PREVIOUS_METRICS_PATH}) が見つかりません。初回実行または比較対象なしとして扱います。"
        )
    ...
    previous_inference_time_avg = previous_metrics.get(
        "inference_time_avg_ms_per_sample"
    )
    ...
    if (
        previous_inference_time_avg is not None
        and current_inference_time_avg is not None
        and previous_inference_time_avg > 0
    ):
        # 推論時間が著しく増加していないか (例: 過去の1.5倍を超えたらエラー)
        assert (
            current_inference_time_avg <= previous_inference_time_avg * 1.5
...
def pytest_sessionfinish(session, exitstatus):
    ...
    if (
        exitstatus == 0 and collected_metrics
    ):  # テストが成功し、かつメトリクスが収集されている場合
